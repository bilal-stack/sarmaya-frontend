...
export function InvestorPitchCoachChatList() {
    const { user } = useAuth();
    ...
    const fetchChatRooms = useCallback(async (page = 1, continuationToken: string | null = null) => {
        ...
        try {
            let url = `http://localhost:8000/api/v1/investor-pitch-coach?page=${page}&per_page=15`;
            ...
            setChatRooms([]); // Clear chat rooms on error
        } finally {
            setIsLoading(false);
            setIsFetchingMore(false);
        }
